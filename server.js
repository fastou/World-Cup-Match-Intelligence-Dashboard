...
const SOCCER_COMPETITION_DEFINITIONS = [
  ...
  { key: "euro", categoryKey: "international", label: "欧洲杯", labelEn: "UEFA Euro", tier: 1, teamType: "national", stageType: "tournament", patterns: ["euro 202", "euro-202", "european championship"] },
  ...
  { key: "uefa-conference-league", categoryKey: "europeanClub", label: "欧协联", labelEn: "UEFA Conference League", tier: 2, teamType: "club", stageType: "uefa-club", patterns: ["conference league", "uefa conference", "uecl-", "europa-conference-league"] },
  ...
  { key: "liga-mx", categoryKey: "americas", label: "墨西哥联赛", labelEn: "Liga MX", tier: 2, teamType: "club", stageType: "league", patterns: ["liga mx", "liga-mx", "mex-"] },
  { key: "libertadores", categoryKey: "americas", label: "解放者杯", labelEn: "Copa Libertadores", tier: 1, teamType: "club", stageType: "continental-club", patterns: ["libertadores", "copa libertadores", "lib-202"] },
  { key: "sudamericana", categoryKey: "americas", label: "南美杯", labelEn: "Copa Sudamericana", tier: 2, teamType: "club", stageType: "continental-club", patterns: ["sudamericana", "copa sudamericana", "sud-202"] },
...
function baseSoccerEventSlug(slug) {
  return String(slug || "")
    .replace(/-(more-markets|exact-score|player-props|halftime-result|first-to-score|second-half-result|both-teams-to-score|team-total|total-goals|spread|corners?|goalscorer|shots?|assists?)$/i, "");
}

function isDerivativeSoccerGameEvent(event = {}) {
  const title = String(event.title || "");
  const slug = String(event.slug || "");
  return /-(more-markets|exact-score|player-props|halftime-result|first-to-score|second-half-result|both-teams-to-score|team-total|total-goals|spread|corners?|goalscorer|shots?|assists?)$/i.test(slug)
    || /\s+-\s+(more markets|exact score|correct score|player props|halftime result|first team to score|second half result|both teams to score|team total|total goals|spread|corners?|goalscorer|shots?|assists?)/i.test(title);
}
...
function isBaseSoccerGameEvent(event = {}) {
  ...
  if (isDerivativeSoccerGameEvent(event)) return false;
...
function soccerEventShanghaiDateKey(event = {}) {
  const kickoffMs = dateMs(soccerEventKickoffIso(event));
  return kickoffMs ? shanghaiDateKey(new Date(kickoffMs)) : "";
}

function selectPolymarketVisibleBaseEvents(events = [], nowMs = Date.now()) {
  const byDate = new Map();
  for (const event of events || []) {
    if (!shouldIncludePolymarketSoccerGame(event, nowMs)) continue;
    const dateKey = soccerEventShanghaiDateKey(event) || "unknown";
  ...
  for (const dateKey of [...byDate.keys()].sort()) {
    const dayEvents = byDate.get(dateKey)
      .sort((a, b) =>
        soccerEventVolume(b) - soccerEventVolume(a)
        || (dateMs(soccerEventKickoffIso(a)) || 0) - (dateMs(soccerEventKickoffIso(b)) || 0)
        || String(a.title || "").localeCompare(String(b.title || ""))
      );
    selected.push(...dayEvents.slice(0, POLYMARKET_SOCCER_BASELINE_MATCHES_PER_DAY));
  }
  return selected
    .sort((a, b) =>
      (dateMs(soccerEventKickoffIso(a)) || 0) - (dateMs(soccerEventKickoffIso(b)) || 0)
...
function shanghaiDateKeyToUtcMs(key, endOfDay = false) {
  const value = String(key || "");
  if (!/^\d{8}$/.test(value)) return null;
  const iso = `${value.slice(0, 4)}-${value.slice(4, 6)}-${value.slice(6, 8)}T${endOfDay ? "15:59:59.999" : "16:00:00.000"}Z`;
  const ms = new Date(iso).getTime();
  return Number.isFinite(ms) ? ms : null;
}

function shanghaiWindowEndMs(nowMs = Date.now(), days = MATCH_WINDOW_DAYS) {
  const endKey = shanghaiDateKey(addDays(new Date(nowMs), days));
  return shanghaiDateKeyToUtcMs(endKey, true) || (nowMs + days * 86400000);
}
...
function inUpcomingWindow(kickoffIso, nowMs = Date.now(), days = MATCH_WINDOW_DAYS) {
  ...
  const windowEnd = shanghaiWindowEndMs(nowMs, days);
...
function isNationalTeamKnockoutMatch(match) {
  return !isClubMatch(match) && isKnockoutMatch(match);
}
...
function matchShapeProfile(match, options = {}) {
  ...
  const btts = Number(probabilities.btts) || 0;
  ...
  const oneNilMass = scoreProbability(match, "1-0") + scoreProbability(match, "0-1");
  const nilNilMass = scoreProbability(match, "0-0");
  const oneOneMass = scoreProbability(match, "1-1");
  const drawScoreMass = scoreMass(match, (homeGoals, awayGoals) => homeGoals === awayGoals && homeGoals + awayGoals <= 2);
  const lowScoreMass = scoreMass(match, (homeGoals, awayGoals) => homeGoals + awayGoals <= 2);
  const deepScoreMass = scoreMass(match, (homeGoals, awayGoals) => homeGoals + awayGoals >= 4);
  const likelyLowEvent = under25 >= 0.54 || lowScoreMass >= 0.48 || over25 <= 0.46;
  const likelyDrawPath = draw >= 0.27 || drawScoreMass >= 0.18 || nilNilMass + oneOneMass >= 0.16;
  const balanced = favoriteGap <= (round16Plus ? 0.18 : 0.14);
  const marketBias = correctScoreMarketBias(match);
  const marketBalanced = !marketBias.regularFavorite || marketBias.regularMargin <= 0.14;
  ...
  if (knockout && draw >= 0.3) {
    notes.push("淘汰赛 90 分钟平局概率较高，晋级优势不能直接等同于90分钟赢球。");
    notesEn.push("Knockout regulation draw is material; to-advance edge must not be treated as a 90-minute win.");
  }
  if (likelyLowEvent && btts < 0.5) {
    notes.push("低事件/零封路径权重较高，BTTS 与深比分默认降级。");
    notesEn.push("Low-event and clean-sheet paths are elevated; BTTS and deep scores are downgraded by default.");
  }
  return {
    key: balancedKnockoutLowScore ? "balanced_knockout_low_score"
      : knockout && likelyLowEvent ? "knockout_low_score"
        : knockout ? "knockout_open"
          : likelyLowEvent ? "low_score"
            : "standard",
    label: balancedKnockoutLowScore ? "均势淘汰赛低比分"
      : knockout && likelyLowEvent ? "淘汰赛低比分"
        : knockout ? "淘汰赛常规"
          : likelyLowEvent ? "低比分"
            : "常规",
    labelEn: balancedKnockoutLowScore ? "Balanced knockout low-score"
      : knockout && likelyLowEvent ? "Knockout low-score"
        : knockout ? "Knockout standard"
          : likelyLowEvent ? "Low-score"
            : "Standard",
    knockout,
    round16Plus,
    balanced,
    ...
    likelyLowEvent,
    likelyDrawPath,
    balancedKnockoutLowScore,
    topScoreIsDraw,
    favoriteGap: roundTo(favoriteGap, 4),
    regularMarketMargin: roundTo(marketBias.regularMargin || 0, 4),
    drawProbability: roundTo(draw, 4),
    under25: roundTo(under25, 4),
    btts: roundTo(btts, 4),
    nilNilMass: roundTo(nilNilMass, 4),
    oneOneMass: roundTo(oneOneMass, 4),
    oneNilMass: roundTo(oneNilMass, 4),
    drawScoreMass: roundTo(drawScoreMass, 4),
    lowScoreMass: roundTo(lowScoreMass, 4),
    deepScoreMass: roundTo(deepScoreMass, 4),
    scoreLeader,
    scoreLeaderSide,
...
function probabilitySnapshot(probabilities = {}) {
  return {
    home: roundTo(probabilities.home, 4),
    draw: roundTo(probabilities.draw, 4),
    away: roundTo(probabilities.away, 4),
...
function historicalShapeCalibration(match, probabilities, options = {}) {
  ...
  const lowScoreMass = scoreDistributionMass(probabilities, (homeGoals, awayGoals) => homeGoals + awayGoals <= 2);
  const drawLowMass = scoreDistributionMass(probabilities, (homeGoals, awayGoals) => homeGoals === awayGoals && homeGoals + awayGoals <= 2);
  const oneNilMass = scoreDistributionMass(probabilities, (homeGoals, awayGoals) => homeGoals + awayGoals === 1);
  const threePlusMass = scoreDistributionMass(probabilities, (homeGoals, awayGoals) => homeGoals + awayGoals >= 3);
  const deltas = { home: 0, draw: 0, away: 0, over25: 0, btts: 0 };
  const notes = [];
  const missing = [];
  ...
  if (!Number.isFinite(xgGap) || !Number.isFinite(lambdaSum)) {
    ...
    if (dataClarity.unknownBalance) {
      ...
      notes.push("历史形态：xG 几乎均势且总强度不高时，实际更容易落入 0-0/1-1/一球小胜簇。");
    } else if (xgGap >= 0.2 && xgGap < 0.45 && favorite.favorite >= 0.38 && favorite.favorite <= 0.58) {
      deltas.btts += 0.052 * shapeScale;
      deltas.over25 += 0.024 * shapeScale;
      deltas.draw += 0.004 * shapeScale;
    ...
    if (xgGap >= 0.75 || (favorite.favorite >= 0.62 && xgGap >= 0.55)) {
      const sideBoost = 0.008 * shapeScale;
      deltas[favorite.key] += sideBoost;
      deltas.draw -= 0.012 * shapeScale;
      deltas[favorite.key === "home" ? "away" : "home"] -= Math.max(0, sideBoost - 0.002);
      deltas.over25 += 0.044 * shapeScale;
      deltas.btts += 0.014 * shapeScale;
      notes.push("历史形态：明显热门更容易带出第三球以上，不能自动把强队优势解释成零封小球。");
    }

    if (lambdaSum < 2.22 && xgGap < 0.35) {
      deltas.over25 -= 0.018 * shapeScale;
      deltas.btts -= 0.008 * shapeScale;
      notes.push("总 xG 偏低且差距不大时，大球/BTTS 需要现场节奏确认。");
    }
  }

  if (knockout && lowScoreMass >= 0.5 && favorite.gap <= 0.34) {
    deltas.draw += 0.006;
    deltas.over25 -= 0.008;
    notes.push("淘汰赛形态：低比分簇占优且强弱差不大，90分钟平局保护继续保留。");
  }

  if (oneNilMass >= 0.2 && probabilities.btts < 0.52 && !tone.isMls && !tone.isLigaMx) {
    deltas.btts -= 0.006;
    notes.push("一球零封路径占比偏高，BTTS Yes 只保留小幅概率，不直接主推。");
  }

  if (threePlusMass >= 0.45 && probabilities.over25 < 0.5) {
    deltas.over25 += 0.01;
    notes.push("比分分布已有 3球+ 簇，但汇总大球偏低，做同源分布回补。");
  }
  ...
  const adjusted = applyProbabilityDeltasThroughScoreGrid(probabilities, deltas, { markTrendAdjusted: true });
  return {
    ok: true,
    version: "2026.07.24",
    probabilities: adjusted,
    ...
    shape: {
      ...
      favoriteProbability: roundTo(favorite.favorite, 4),
      lowScoreMass: roundTo(lowScoreMass, 4),
    ...
    missing: [...new Set(missing)].slice(0, 4),
    before: probabilitySnapshot(probabilities),
    after: probabilitySnapshot(adjusted)
  };
}
...
function clubCompetitionTone(match = {}) {
  const competition = match?.competition || {};
  const text = [
    competition.key,
    competition.parentKey,
    competition.seriesSlug,
    competition.stageType,
    competition.stageLabel,
    competition.label,
    competition.labelEn,
  ...
  const lowRatingConfidence = profiles.length < 2 || profiles.some((profile) => {
    ...
    return profile?.teamType === "club" && (!profile.override || ["low", "very-low", "fallback", "unknown"].includes(confidence));
  });
  const homeForm = recentFormStrength(recentFormSummary(match, "home"));
  const awayForm = recentFormStrength(recentFormSummary(match, "away"));
  const recentThin = !homeForm || !awayForm || Math.min(homeForm.matches || 0, awayForm.matches || 0) < 3;
  ...
  return {
    ...
    isQualifier,
    isAmericas,
    lowData: isClub && (lowRatingConfidence || recentThin || dynamicThin),
    lowRatingConfidence,
    recentThin,
    dynamicThin,
...
const CLUB_COMPETITION_BACKTEST_PROFILES = {
  mls: {
    key: "mls",
    label: "MLS",
    sample: 30,
    mainDirectionHitRate: 0.533,
    bttsRate: 0.567,
    over25Rate: 0.567,
    drawRate: 0.233,
    avgGoals: 2.67,
...
function clubEmpiricalBacktestProfile(tone = {}) {
  if (!tone.isClub) return null;
  const profile = tone.isMls
    ? CLUB_COMPETITION_BACKTEST_PROFILES.mls
    : tone.isLigaMx
      ? CLUB_COMPETITION_BACKTEST_PROFILES.ligaMx
      : tone.isCopaSudamericana
        ? CLUB_COMPETITION_BACKTEST_PROFILES.sudamericana
        : tone.isUcl && tone.isQualifier
          ? CLUB_COMPETITION_BACKTEST_PROFILES.uclQualifier
          : tone.isUel && tone.isQualifier
            ? CLUB_COMPETITION_BACKTEST_PROFILES.uelQualifier
            : tone.isUecl && tone.isQualifier
              ? CLUB_COMPETITION_BACKTEST_PROFILES.ueclQualifier
              : null;
  if (!profile) return null;
  const sampleWeight = clamp((profile.sample || 0) / 30, 0.45, 1);
  return {
    ...profile,
    sampleWeight,
    note: `${profile.label} 近 ${profile.sample} 场临时匹配回测：主方向 ${formatPercent(profile.mainDirectionHitRate)}，BTTS ${formatPercent(profile.bttsRate)}，大2.5 ${formatPercent(profile.over25Rate)}，平局 ${formatPercent(profile.drawRate)}。`
  };
}
...
function clubBttsReviewProfile(tone = {}) {
  ...
  if (tone.isMls) {
    return {
      ...
      bttsDelta: 0.025,
      over25Delta: 0.012,
      notes: ["BTTS复盘校准：MLS 严格已结算样本仍偏少，不再用宽口径样本强上修；只保留小幅开放倾向。"]
    };
  }
  if (tone.isLigaMx) {
    return {
      ...
      bttsDelta: 0.018,
      over25Delta: 0.01,
      notes: ["BTTS复盘校准：墨西哥联赛样本太薄，只保留轻微开放倾向，不直接强推。"]
    };
  }
  if (tone.isCopaSudamericana) {
    return {
      key: "sudamericana-btts-neutral",
      bttsDelta: -0.012,
      over25Delta: -0.004,
      notes: ["BTTS复盘校准：南美杯严格样本不足以形成强规则，只做轻微降权，核心仍看球队形态和现场数据。"]
    };
  }
  if (tone.isUcl && tone.isQualifier) {
    return {
      ...
      bttsDelta: -0.025,
      over25Delta: -0.01,
      notes: ["BTTS复盘校准：欧冠资格赛近期偏低双进，但资格赛数据噪声大，只做保守降权。"]
    };
  }
  if ((tone.isUel || tone.isUecl) && tone.isQualifier) {
    return {
      key: "uel-uecl-qualifier-cautious",
      bttsDelta: -0.018,
      over25Delta: -0.006,
      notes: ["BTTS复盘校准：欧联/欧协资格赛严格样本偏低双进，保持谨慎，不按联赛开放节奏处理。"]
    };
  }
...
function clubCompetitionReviewAdjustment(match, probabilities) {
  if (!isClubMatch(match) || !probabilities?.topScoresFull?.length) {
  ...
  if (tone.isMls) {
    ...
    notes.push("南美杯按单独节奏处理：不套用 MLS/墨西哥联赛的开放校准，重点看两回合形势和球队近况。");
  } else if (tone.isLeague || tone.isAmericas) {
    overBoost += tone.isAmericas ? 0.01 : 0.008;
    bttsBoost += tone.isAmericas ? 0.01 : 0.008;
    notes.push("俱乐部联赛/美洲赛事只保留小幅开放先验，具体方向交给球队形态、近况和盘口分歧校验。");
  }
  if (tone.isEuropeanClub) {
    ...
    notes.push(tone.isQualifier
      ...
      : "欧洲俱乐部赛按俱乐部强度处理，不套用国家队世界杯淘汰赛经验。");
  }
  if (tone.lowData) {
    drawBoost += 0.007;
    bttsBoost += 0.007;
    overBoost += 0.005;
    if (favorite.favorite >= 0.42) favoriteWinPenalty += favorite.gap <= 0.28 ? 0.012 : 0.007;
  ...
  if (empirical) {
    ...
    if (overDelta > 0) overBoost += overDelta;
    else probabilityDeltas.over25 += overDelta;
    if (drawDelta > 0) drawBoost += drawDelta;
    if (empirical.mainDirectionHitRate >= 0.58 && favorite.favorite >= 0.42) {
      favoriteWinBoost += clamp((empirical.mainDirectionHitRate - 0.56) * 0.13 * weight, 0.004, 0.014);
  ...
  favoriteWinBoost = clamp(favoriteWinBoost, 0, 0.02);
  ...
  if (!drawBoost && !bttsBoost && !overBoost && !favoriteWinPenalty && !favoriteWinBoost) {
  ...
  let adjusted = reweightScoreDistribution(probabilities, (score) => {
    ...
    let weight = 1;
    if (homeGoals === awayGoals) weight += drawBoost;
    if (homeGoals > 0 && awayGoals > 0) weight += bttsBoost;
    if (totalGoals >= 3) weight += overBoost;
    if (favoriteKey === "home" && homeGoals > awayGoals) weight += favoriteWinBoost;
    if (favoriteKey === "away" && awayGoals > homeGoals) weight += favoriteWinBoost;
  ...
  return {
    ...
    probabilities: adjusted,
    tone,
    backtestProfile: empirical ? {
      key: empirical.key,
      label: empirical.label,
      sample: empirical.sample,
      mainDirectionHitRate: empirical.mainDirectionHitRate,
      bttsRate: empirical.bttsRate,
      over25Rate: empirical.over25Rate,
      drawRate: empirical.drawRate,
      avgGoals: empirical.avgGoals,
...
function knockoutReviewAdjustment(match, probabilities, contextSignals = null) {
  if (!isNationalTeamKnockoutMatch(match) || !probabilities?.topScoresFull?.length) {
...
function buildGoldmanStyleModel(match, fifaRankings = {}, { useMarketCalibration = false } = {}) {
  ...
  const clubReview = clubCompetitionReviewAdjustment(match, probabilities);
  if (clubReview.ok) {
    probabilities = clubReview.probabilities;
    match.clubCompetitionAdjustment = {
      version: "2026.07.27-club-empirical-profile",
      tone: clubReview.tone,
      backtestProfile: clubReview.backtestProfile,
    ...
    drivers.push({
      ...
      awayXgDelta: 0,
      reason: [
        `按俱乐部赛事类型和数据质量统一修正：平局 ${clubReview.deltas.draw >= 0 ? "+" : ""}${formatPercent(clubReview.deltas.draw)}，大2.5 ${clubReview.deltas.over25 >= 0 ? "+" : ""}${formatPercent(clubReview.deltas.over25)}，BTTS ${clubReview.deltas.btts >= 0 ? "+" : ""}${formatPercent(clubReview.deltas.btts)}。`,
        ...clubReview.notes.slice(0, 2)
      ].join(" ")
    });
  }
  ...
  if (historicalShape.ok) {
    ...
    match.historicalShapeCalibration = {
      version: historicalShape.version,
      deltas: historicalShape.deltas,
      shape: historicalShape.shape,
      before: historicalShape.before,
      after: historicalShape.after,
    ...
    drivers.push({
      ...
      awayXgDelta: 0,
      reason: [
        `按历史赛果/现场回测形态统一修正：平局 ${historicalShape.deltas.draw >= 0 ? "+" : ""}${formatPercent(historicalShape.deltas.draw)}，大2.5 ${historicalShape.deltas.over25 >= 0 ? "+" : ""}${formatPercent(historicalShape.deltas.over25)}，BTTS ${historicalShape.deltas.btts >= 0 ? "+" : ""}${formatPercent(historicalShape.deltas.btts)}。`,
        ...historicalShape.notes.slice(0, 2)
      ].join(" ")
    });
  }
  ...
  return {
    ...
    historicalShapeCalibration: historicalShape.ok ? {
      version: historicalShape.version,
      deltas: historicalShape.deltas,
      shape: historicalShape.shape,
      before: historicalShape.before,
...
function marketReviewAdjustments(match, rec) {
  ...
  if (clubTone.isClub && ["total", "btts"].includes(rec.marketType) && typeof rec.marketPrice === "number") {
    if ((clubTone.isMls || clubTone.isLigaMx) && rec.key === "under25") {
      edgePenalty += rec.marketPrice <= 0.5 ? 0.055 : 0.035;
      scorePenalty += rec.marketPrice <= 0.5 ? 0.065 : 0.045;
      reasons.push(clubTone.isMls
        ? "MLS 严格样本不足以单独否定小球；但赛前小球需要球队形态、盘口和现场节奏共同支持。"
        : "墨西哥联赛样本太薄，小球不能按低比分模板主推，需等待更多证据。");
    }
    if ((clubTone.isMls || clubTone.isLigaMx) && rec.key === "bttsNo") {
      edgePenalty += clubTone.isMls ? (rec.marketPrice <= 0.55 ? 0.07 : 0.045) : (rec.marketPrice <= 0.55 ? 0.055 : 0.038);
      scorePenalty += clubTone.isMls ? (rec.marketPrice <= 0.55 ? 0.085 : 0.055) : (rec.marketPrice <= 0.55 ? 0.065 : 0.045);
      reasons.push(clubTone.isMls
        ? "MLS 不能用宽口径样本直接禁止 BTTS No，但赛前零封判断必须有阵容/现场射门证据。"
        : "墨西哥联赛 BTTS No 先降级，等待更多样本或现场证据。");
    }
    if ((clubTone.isMls || clubTone.isLigaMx) && (rec.key === "over25" || rec.key === "bttsYes") && rec.marketPrice <= 0.67) {
      scorePenalty = Math.max(0, scorePenalty - (clubTone.isMls ? 0.018 : 0.026));
      edgePenalty = Math.max(0, edgePenalty - (clubTone.isMls ? 0.008 : 0.014));
      reasons.push(clubTone.isMls
        ? "MLS 只给 Over/BTTS Yes 小幅观察加权，仍按价格、形态和首发纪律执行。"
        : "墨西哥联赛近期 BTTS 偏高，Over/BTTS Yes 可提高观察优先级，但仍按价格执行。");
    }
    if (clubTone.isCopaSudamericana && rec.key === "bttsYes") {
      if (rec.marketPrice <= 0.64 && btts >= 0.46) {
        edgePenalty = Math.max(0, edgePenalty - 0.01);
        scorePenalty = Math.max(0, scorePenalty - 0.022);
        reasons.push("南美杯近期偏开放，BTTS Yes 不再默认降级；但样本小，价格过高仍不追。");
      } else {
        edgePenalty += 0.012;
        scorePenalty += 0.018;
        reasons.push("南美杯 BTTS Yes 只在价格和球队形态都支持时进入观察，不能单靠赛事样本。");
      }
    }
    if (clubTone.isCopaSudamericana && rec.key === "bttsNo" && rec.marketPrice <= 0.58) {
      edgePenalty += 0.032;
      scorePenalty += 0.046;
      reasons.push("南美杯近期大球/双进偏热，BTTS No 需要明确零封证据才看。");
    }
    if (clubTone.isCopaSudamericana && rec.key === "over25") {
      edgePenalty = Math.max(0, edgePenalty - 0.016);
      scorePenalty = Math.max(0, scorePenalty - 0.03);
      reasons.push("南美杯近样本大2.5偏高，Over 2.5 观察优先级上修。");
    }
    if (clubTone.isCopaSudamericana && rec.key === "under25") {
      edgePenalty += 0.038;
      scorePenalty += 0.06;
      reasons.push("南美杯近样本大球率偏高，小球赛前主推门槛提高。");
    }
    if (clubTone.isUcl && clubTone.isQualifier && rec.key === "bttsYes") {
      edgePenalty += 0.032;
      scorePenalty += 0.045;
      reasons.push("欧冠资格赛严格样本偏低双进，赛前 BTTS Yes 需要价格或阵容强证据。");
    }
    ...
    if (clubTone.isEuropeanClub && clubTone.isQualifier && rec.key === "under25") {
      ...
      const uclColdProfile = clubTone.isUcl && clubEmpirical?.over25Rate <= 0.42;
      edgePenalty += uclColdProfile ? (extremeQualifierUnder ? 0.06 : 0.025) : (extremeQualifierUnder ? 0.12 : 0.07);
      scorePenalty += uclColdProfile ? (extremeQualifierUnder ? 0.07 : 0.035) : (extremeQualifierUnder ? 0.12 : 0.075);
      if (!uclColdProfile && (clubLowData || extremeQualifierUnder)) forceNoBuy = true;
      if (extremeQualifierUnder) forceNoBuy = true;
      reasons.push(uclColdProfile
        ? "欧冠资格赛近样本偏低双进/低大球，小球可观察，但极端低价仍需核验盘口。"
        : "低数据欧战资格赛复盘：小球 edge 容易被盘口口径/早球风险放大，必须先核验真实盘口和阵容。");
    }
    if (clubLowData && rec.key === "under25" && rec.marketPrice <= 0.46) {
      ...
      scorePenalty += strongMarketConflict ? 0.12 : 0.09;
      forceNoBuy = true;
    ...
    if ((clubTone.isLeague || clubTone.isAmericas) && rec.key === "under25" && rec.marketPrice <= 0.5) {
      edgePenalty += 0.035;
      scorePenalty += 0.045;
      reasons.push("俱乐部联赛/美洲赛事节奏更开放，小球赛前主推门槛提高。");
    }
    if ((clubTone.isLeague || clubTone.isAmericas) && (rec.key === "over25" || rec.key === "bttsYes") && rec.marketPrice >= 0.54) {
      scorePenalty = Math.max(0, scorePenalty - 0.012);
      reasons.push("俱乐部联赛/美洲赛事与市场同向偏开放，Over/BTTS Yes 可进入观察队列。");
    }
  }

  if (clubLowData && rec.marketType === "moneyline" && typeof rec.edge === "number") {
    const moneyline = match?.manualMarkets?.moneyline || {};
  ...
  if (clubEmpirical && rec.marketType === "moneyline" && typeof rec.edge === "number") {
    if (rec.key === favoriteKey && clubEmpirical.mainDirectionHitRate >= 0.58 && rec.modelProbability >= 0.5 && rec.edge >= 0.06) {
      edgePenalty = Math.max(0, edgePenalty - 0.01);
    ...
    if (rec.key === "draw" && clubEmpirical.drawRate >= 0.3 && rec.edge >= 0.035) {
      ...
      reasons.push(`${clubEmpirical.label} 近样本平局率偏高，平局可作为观察路径。`);
    }
  }
  ...
  const hasPolymarketCurve = rec.chart?.source === "Polymarket" && Array.isArray(rec.chart.history) && rec.chart.history.length >= 2;
  const positivePreEdge = preMatch
    && typeof rec.edge === "number"
    && rec.edge >= 0.06
    && typeof rec.marketPrice === "number"
    && typeof rec.modelProbability === "number";
  if (positivePreEdge) {
    const modelMarketGap = rec.modelProbability - rec.marketPrice;
    if (["total", "btts"].includes(rec.marketType) && modelMarketGap >= 0.14) {
      const severeGap = modelMarketGap >= 0.22 || rec.marketPrice <= 0.18;
      edgePenalty += severeGap ? 0.085 : 0.052;
      scorePenalty += severeGap ? 0.105 : 0.07;
      if ((clubLowData || (chartStatusKnown && !hasPolymarketCurve)) && severeGap) forceNoBuy = true;
    ...
    if (rec.marketType === "moneyline" && rec.key === "draw" && modelMarketGap >= 0.1 && rec.marketPrice <= 0.22) {
      edgePenalty += 0.035;
      scorePenalty += 0.05;
      reasons.push("历史盘口分歧校准：低价平局虽有价值空间，但样本波动大，只能观察/小仓。");
    }
  }
...
function sortRecommendationRows(rows = [], match = null) {
  for (const rec of rows) {
    rec.displaySortScore = roundTo(tradableRecommendationScore(rec, match), 4);
  }
  return rows.sort((a, b) => (b.displaySortScore ?? -9) - (a.displaySortScore ?? -9));
}
...
function buildRecommendations(match, probabilities) {
  ...
  return sortRecommendationRows(rows
    .map((row) => {
    ...
    .map((row) => applyReviewDiscipline(row, match)), match);
}
...
function correctScoreStrategy(row, match) {
  ...
  const shape = matchShapeProfile(match);
  ...
  if (shape.balancedKnockoutLowScore) {
    if (row.score === "0-0" || row.score === "1-1") {
      alignmentBoost += row.score === "0-0" ? 0.085 : 0.072;
      rankBoost += 0.025;
      reasons.push("均势淘汰赛低比分盘型：0-0/1-1 是主轴，不等到中途才补救。");
    } else if ((row.score === "1-0" || row.score === "0-1") && row.modelProbability >= 0.025) {
      alignmentBoost += 0.042;
      reasons.push("均势淘汰赛低比分盘型：两边一球小胜只做保护翼。");
    }
    if (side === "draw" && goals <= 2) {
      alignmentBoost += 0.035;
    }
    if (goals >= 3) {
      directionPenalty += 0.045 + Math.max(0, goals - 3) * 0.018;
      reasons.push("均势淘汰赛低比分盘型下，深比分默认降级，等待现场打开后再考虑。");
    }
    if (side && side !== "draw" && side === bias.regularFavorite && bias.regularMargin <= 0.1) {
      directionPenalty += 0.015;
      reasons.push("盘口接近均势，不能把轻微热门直接放大成90分钟胜主仓。");
    }
  }
  ...
  let tier = "watch";
  if (shape.balancedKnockoutLowScore) {
    if (["0-0", "1-1"].includes(row.score) && row.modelProbability >= 0.035) tier = "primary";
    else if (["1-0", "0-1"].includes(row.score) && row.modelProbability >= 0.025) tier = "secondary";
    else if (goals >= 3) tier = row.modelProbability >= 0.025 && baseEdge >= 0.035 ? "longshot" : "avoid";
    else if (opposed) tier = "avoid";
    else if (row.scoreRank <= 5) tier = "watch";
  } else if (row.modelProbability < 0.03) tier = side && side === bias.regularFavorite ? "longshot" : "watch";
  else if (opposed) tier = "avoid";
...
function correctScoreComboCandidateValue(row, match, options = {}) {
  ...
  const shape = options.shape || matchShapeProfile(match);
  ...
  if (shape.balancedKnockoutLowScore) {
    if (row.score === "0-0") value += 0.105;
    else if (row.score === "1-1") value += 0.092;
    else if (row.score === "1-0" || row.score === "0-1") value += 0.05;
    if (side === "draw" && goals <= 2) value += 0.035;
    if (goals >= 3) value -= 0.075 + Math.max(0, goals - 3) * 0.025;
    if (margin >= 2) value -= 0.035;
  }
  return value;
}
...
function correctScoreComboRows(rows = [], match, options = {}) {
  ...
  const preferred = validRows.filter((row) => {
    ...
    if (shape.balancedKnockoutLowScore) {
      const goals = (Number(row.homeGoals) || 0) + (Number(row.awayGoals) || 0);
      const margin = Math.abs((Number(row.homeGoals) || 0) - (Number(row.awayGoals) || 0));
      if (goals >= 4 || margin >= 3) return false;
    }
    return true;
  });
...
function rowByScore(rows = [], score) {
  return rows.find((row) => row?.score === score) || null;
}

function buildBalancedLowScoreCorrectScoreCombo(rows, match, options = {}) {
  const shape = options.shape || matchShapeProfile(match);
  if (!shape.balancedKnockoutLowScore) return null;
  const coreScores = ["0-0", "1-1", "1-0", "0-1"];
  const selected = uniqueScoreRows(coreScores.map((score) => rowByScore(rows, score)).filter(Boolean));
  if (selected.length < 3) return null;
  const totalBudgetPct = options.live ? 0.5 : 0.58;
  const cashReservePct = roundTo(1 - totalBudgetPct, 4);
  const rawWeights = selected.map((row) => {
    if (row.score === "0-0") return 0.3;
    if (row.score === "1-1") return 0.3;
    if (row.score === "1-0") return 0.2;
    if (row.score === "0-1") return 0.2;
    return 0.08;
  });
  const sumWeights = rawWeights.reduce((sum, value) => sum + value, 0);
  const roleMap = {
    "0-0": ["低事件主轴", "Low-event core"],
    "1-1": ["平局进球保护", "Scored-draw cover"],
    "1-0": [`${match.homeName}一球保护`, "Home one-goal cover"],
    "0-1": [`${match.awayName}一球保护`, "Away one-goal cover"]
  };
  const legs = selected.map((row, index) => {
    const [role, roleEn] = roleMap[row.score] || ["保护", "Cover"];
    const reason = row.score === "0-0" || row.score === "1-1"
      ? "盘口和模型接近均势时，先覆盖平局低事件主轴。"
      : "两边一球小胜只做保护翼，不当作赛前主仓。";
    return correctScoreLeg(
      row,
      role,
  ...
  return {
    key: "balanced-knockout-low-score-cluster",
    title: options.live ? "实时低比分平局簇" : "均势淘汰赛低比分簇",
    titleEn: options.live ? "Live low-score draw cluster" : "Balanced knockout low-score cluster",
    method: "Shape-first cover",
    methodEn: "Shape-first cover",
    riskLevel: "先防路径错配",
    riskLevelEn: "Prevents path mismatch first",
    totalBudgetPct,
    cashReservePct,
    summary: options.live
      ? "现场仍呈低事件均势时，优先管理 0-0/1-1 和两边一球路径；不把绝杀方向当主仓。"
      : "均势淘汰赛先从 0-0/1-1 平局簇开始，再用 1-0/0-1 做两翼保护，剩余现金等20分钟后的节奏确认。",
    summaryEn: options.live
      ? "When live play remains balanced and low-event, manage 0-0/1-1 plus one-goal covers; do not make a late winner the core path."
      : "For balanced knockout matches, start from the 0-0/1-1 draw cluster, add 1-0/0-1 one-goal covers, and keep reserve for the 20-minute tempo check.",
    metrics,
    legs,
    rules: [
      "赛前只用该场球胆预算的约一半，剩余等 18-25 分钟确认节奏。",
      "若 20 分钟内节奏快、射正多或早球出现，停止补 0-0，转入现场轮动。",
      "若半场仍低事件，优先加当前比分/平局保护，而不是补单边胜。"
    ],
    rulesEn: [
      "Use only about half of the match correct-score budget pre-match; reserve the rest for the 18-25' tempo check.",
      "If tempo is fast, shots on target are high, or an early goal arrives, stop adding 0-0 and rotate live.",
      "If half-time remains low-event, add current-score or draw cover first rather than a one-side win."
    ],
    shape
  };
}
...
function buildEqualReturnCorrectScoreCombo(rows, match, options = {}) {
  const round16Plus = Boolean(options.round16Plus);
  const shape = options.shape || matchShapeProfile(match);
  const totalBudgetPct = shape.balancedKnockoutLowScore ? 0.5 : round16Plus ? 0.6 : 1;
  const cashReservePct = roundTo(1 - totalBudgetPct, 4);
  const candidates = rows
    .filter((row) => row.modelProbability >= (round16Plus ? 0.025 : 0.035) || row.scoreRank <= (round16Plus ? 8 : 6))
    .sort((a, b) => {
      return correctScoreComboCandidateValue(b, match, { round16Plus, shape }) - correctScoreComboCandidateValue(a, match, { round16Plus, shape });
    });
  ...
  return {
    ...
    title: shape.balancedKnockoutLowScore ? "均势低比分等回款" : round16Plus ? "16强后等回款 + 留现金" : "等回款覆盖",
    titleEn: shape.balancedKnockoutLowScore ? "Balanced low-score equal-return" : round16Plus ? "Round-of-16 equal-return with reserve" : "Equal-return cover",
    method: "Dutching",
    methodEn: "Dutching",
    ...
    totalBudgetPct,
    cashReservePct,
    summary: shape.balancedKnockoutLowScore
      ? `均势淘汰赛不打满，只按 Yes 价格拆到 ${selected.length} 个低比分/保护比分；先控制路径错配。`
      : round16Plus
    ...
    summaryEn: shape.balancedKnockoutLowScore
      ? `Do not fully allocate in balanced knockout matches; split by Yes price across ${selected.length} low-score or cover scorelines first.`
      : round16Plus
...
function buildCoreCoverCorrectScoreCombo(rows, match, options = {}) {
  const round16Plus = Boolean(options.round16Plus);
  const shape = options.shape || matchShapeProfile(match);
  const totalBudgetPct = shape.balancedKnockoutLowScore ? 0.48 : round16Plus ? 0.55 : 1;
  const cashReservePct = roundTo(1 - totalBudgetPct, 4);
  const byProbability = [...rows].sort((a, b) => round16Plus
    ? correctScoreComboCandidateValue(b, match, { round16Plus, shape }) - correctScoreComboCandidateValue(a, match, { round16Plus, shape })
  ...
  const adjacent = uniqueScoreRows(byProbability
    ...
    .sort((a, b) => {
      const aScore = correctScoreComboCandidateValue(a, match, { round16Plus, shape }) - scoreDistance(a, primary) * (round16Plus ? 0.024 : 0.015);
      const bScore = correctScoreComboCandidateValue(b, match, { round16Plus, shape }) - scoreDistance(b, primary) * (round16Plus ? 0.024 : 0.015);
  ...
  const tail = uniqueScoreRows(rows
    ...
    .sort((a, b) => correctScoreComboCandidateValue(b, match, { round16Plus, shape }) - correctScoreComboCandidateValue(a, match, { round16Plus, shape })))[0];
  const selected = [primary, ...adjacent, tail].filter(Boolean);
  if (selected.length < 2) return null;
  const rawWeights = selected.map((_, index) => (shape.balancedKnockoutLowScore ? [0.34, 0.3, 0.24, 0.12] : round16Plus ? [0.44, 0.28, 0.2, 0.08] : [0.5, 0.25, 0.17, 0.08])[index] || 0.05);
  const sumWeights = rawWeights.reduce((sum, value) => sum + value, 0);
  ...
  return {
    key: "core-cover",
    title: shape.balancedKnockoutLowScore ? "均势主轴 + 两翼保护" : round16Plus ? "16强主路径 + 保护" : "主比分 + 保护",
    titleEn: shape.balancedKnockoutLowScore ? "Balanced core plus two-sided cover" : round16Plus ? "Round-of-16 core path plus cover" : "Core score plus cover",
    method: "Cover bet",
    methodEn: "Cover bet",
    ...
    cashReservePct,
    summary: shape.balancedKnockoutLowScore
      ? "均势淘汰赛主轴优先平局低比分，两边小胜只做保护；不把轻微热门做成赛前主仓。"
      : round16Plus
      ? "16强后主比分不只看模型最高，还看盘口/晋级方向；赛前只放主路径和保护，保留现金等现场确认。"
      : "主比分吃主要收益，旁边两个比分做保险，最后一个深路径只给小尾仓。",
    summaryEn: shape.balancedKnockoutLowScore
      ? "For balanced knockout matches, the core is low-score draw exposure; one-goal wins are covers, not the pre-match core."
      : round16Plus
    ...
    metrics,
    legs,
    rules: shape.balancedKnockoutLowScore ? [
      "如果盘口接近五五开，赛前不要只买一边小胜。",
      "20 分钟后若仍低事件，再考虑加当前比分/平局保护。",
      "若现场明显打开，平局簇停止加仓，转入现场轮动。"
    ] : round16Plus ? [
    ...
    rulesEn: shape.balancedKnockoutLowScore ? [
      "When the market is near even, do not pre-match stack one side's small win only.",
      "After 20 minutes, add current-score or draw cover only if the match remains low-event.",
      "If live play opens up, stop adding the draw cluster and rotate in-play."
    ] : round16Plus ? [
...
function buildBarbellCorrectScoreCombo(rows, match, options = {}) {
  const round16Plus = Boolean(options.round16Plus);
  const shape = options.shape || matchShapeProfile(match);
  const totalBudgetPct = shape.balancedKnockoutLowScore ? 0.3 : round16Plus ? 0.35 : 0.45;
  const cashReservePct = roundTo(1 - totalBudgetPct, 4);
  ...
  const tails = uniqueScoreRows(rows
    ...
    .sort((a, b) => correctScoreComboCandidateValue(b, match, { round16Plus, shape }) - correctScoreComboCandidateValue(a, match, { round16Plus, shape })))
    .slice(0, 2);
  ...
  return {
    key: "barbell-reserve",
    title: shape.balancedKnockoutLowScore ? "均势小仓 + 大留现金" : round16Plus ? "16强小主仓 + 大留现金" : "小主仓 + 长尾 + 留现金",
    titleEn: shape.balancedKnockoutLowScore ? "Balanced small stake and large reserve" : round16Plus ? "Round-of-16 small core and large reserve" : "Small core, tails and reserve",
...
function buildLiveLadderCorrectScoreCombo(rows, match, options = {}) {
  ...
  const currentRows = rows
    ...
    .sort((a, b) => {
      ...
      const aShapeBoost = shape.balancedKnockoutLowScore && ["0-0", "1-1", "1-0", "0-1"].includes(a.score)
        ? a.score === `${score.home}-${score.away}` ? 0.08 : a.score === "0-0" || a.score === "1-1" ? 0.045 : 0.02
        : 0;
      const bShapeBoost = shape.balancedKnockoutLowScore && ["0-0", "1-1", "1-0", "0-1"].includes(b.score)
        ? b.score === `${score.home}-${score.away}` ? 0.08 : b.score === "0-0" || b.score === "1-1" ? 0.045 : 0.02
        : 0;
      const aScore = (a.liveProbability ?? a.modelProbability ?? 0) - aDistance * 0.025 + Math.max(0, a.edge || 0) * 0.2 + aShapeBoost;
      const bScore = (b.liveProbability ?? b.modelProbability ?? 0) - bDistance * 0.025 + Math.max(0, b.edge || 0) * 0.2 + bShapeBoost;
      return bScore - aScore;
    });
  ...
  return {
    key: "live-ladder",
    title: shape.balancedKnockoutLowScore ? "现场低比分轮动" : "现场轮动组合",
    titleEn: shape.balancedKnockoutLowScore ? "Live low-score ladder" : "Live ladder combo",
    ...
    cashReservePct: 0.4,
    summary: shape.balancedKnockoutLowScore
      ? `当前 ${match.homeName} ${score.home}-${score.away} ${match.awayName}。均势低比分盘先管理当前比分和平局/一球保护，下一球后先锁利，不追深比分。`
      : `当前 ${match.homeName} ${score.home}-${score.away} ${match.awayName}，只买还没被打穿的路径；下一球后先锁利再换相邻比分。`,
    summaryEn: shape.balancedKnockoutLowScore
      ? `Current score is ${score.home}-${score.away}. In a balanced low-score shape, manage current-score, draw, and one-goal covers first; trim after the next goal and avoid deep-score chasing.`
      : `Current score is ${score.home}-${score.away}; use only live paths that are not crossed, then trim and rotate after the next goal.`,
    metrics,
    legs,
    rules: [
      "这不是赛前一次性买满，而是下一球后轮动。",
...
function buildCorrectScoreComboStrategy(match, rows = [], options = {}) {
  const shape = matchShapeProfile(match, options);
  ...
  const validRows = correctScoreComboRows(rows, match, { round16Plus, shape });
  ...
  const combos = [
    buildBalancedLowScoreCorrectScoreCombo(validRows, match, { ...options, round16Plus, shape }),
    buildEqualReturnCorrectScoreCombo(validRows, match, { round16Plus, shape }),
    buildCoreCoverCorrectScoreCombo(validRows, match, { round16Plus, shape }),
    buildBarbellCorrectScoreCombo(validRows, match, { round16Plus, shape }),
    options.live ? buildLiveLadderCorrectScoreCombo(validRows, match, { ...options, shape }) : null
  ].filter(Boolean)
    .map((combo) => ({
      ...combo,
      discipline: [
...
function tradableRecommendationScore(rec, match = null) {
  ...
  let score = edgeValue + holderBoost + recommendationMarketPriority(rec) * 0.003 + trendScoreBoost(match, rec) - disciplinePenalty;
  const clubTone = match ? clubCompetitionTone(match) : null;
  const clubEmpirical = clubTone ? clubEmpiricalBacktestProfile(clubTone) : null;
  if (clubTone?.isClub && clubTone.lowData && rec.key === "under25" && typeof rec.marketPrice === "number" && rec.marketPrice <= 0.46) {
    score -= 0.06;
  }
  ...
  if ((clubTone?.isMls || clubTone?.isLigaMx) && rec.key === "under25") {
    score -= clubTone.isMls ? 0.055 : 0.04;
  }
  if ((clubTone?.isMls || clubTone?.isLigaMx) && rec.key === "bttsNo") {
  ...
  if (!clubTone?.isMls && isHighProbabilityClubMoneylinePrimary(match, rec, edgeValue)) {
  ...
  if (rec.decision?.action === "NO_TRADE") {
    score -= 0.04;
  }
  return score;
}
...
function isHighProbabilityClubMoneylinePrimary(match, rec, edgeValue = null) {
  ...
  const tone = clubCompetitionTone(match);
  if (!tone.isClub) return false;
  const empirical = clubEmpiricalBacktestProfile(tone);
  if (!empirical || empirical.mainDirectionHitRate < 0.53) return false;
  const topWinKey = match.probabilities?.home >= match.probabilities?.away ? "home" : "away";
  const probabilityFloor = tone.isMls
    ? 0.68
    : tone.isUcl && tone.isQualifier
      ? 0.56
      : 0.64;
  const edgeFloor = tone.isMls ? 0.06 : tone.isUcl && tone.isQualifier ? 0.065 : 0.08;
  if (effectiveEdge < edgeFloor) return false;
  if (rec.key !== topWinKey || rec.modelProbability < probabilityFloor) return false;
...
function isHighProbabilityMlsMoneylinePrimary(match, rec, edgeValue = null) {
  return clubCompetitionTone(match).isMls && isHighProbabilityClubMoneylinePrimary(match, rec, edgeValue);
}
...
function isPrimaryTradeCandidate(match, rec) {
  ...
  const topWinKey = match.probabilities?.home >= match.probabilities?.away ? "home" : "away";
  if (isHighProbabilityClubMoneylinePrimary(match, rec, edgeValue)) return true;
  return Boolean(
...
function liveScoreNumber(value) {
  return isActualNumber(value) ? Number(value) : null;
}
...
function liveHistoricalStageAdjustment(match, live) {
  const elapsed = elapsedMinuteFromLive(live, match);
  const homeScore = liveScoreNumber(live?.score?.home);
  const awayScore = liveScoreNumber(live?.score?.away);
  if (!Number.isFinite(homeScore) || !Number.isFinite(awayScore)) {
    return { ok: false, homeMultiplier: 1, awayMultiplier: 1, notes: [] };
  }
  ...
  const totalShots = (Number.isFinite(homeShots) ? homeShots : 0) + (Number.isFinite(awayShots) ? awayShots : 0);
  const totalSot = (Number.isFinite(homeSot) ? homeSot : 0) + (Number.isFinite(awaySot) ? awaySot : 0);
  const hasStats = hasUsefulLiveStats(stats);
  const totalGoals = homeScore + awayScore;
  const goalDiff = homeScore - awayScore;
  ...
  if (totalGoals === 0 && elapsed >= 45) {
    const factor = hasStats && (totalSot >= 4 || totalShots >= 16) ? 0.74 : 0.52;
    homeMultiplier *= factor;
    awayMultiplier *= factor;
    notes.push(`${elapsed}' 仍为 0-0，历史现场样本显示后续 BTTS/大2.5 大幅下降；除非射正/禁区压力很强，否则剩余进球降权。`);
  } else if (totalGoals === 0 && elapsed >= 20) {
    const factor = hasStats && (totalSot >= 3 || totalShots >= 12) ? 0.9 : 0.72;
    homeMultiplier *= factor;
    awayMultiplier *= factor;
    notes.push(`${elapsed}' 仍为 0-0，按时间衰减下调后续进球强度，等待18-25分钟后的真实节奏再补仓。`);
  }

  if (totalGoals >= 1 && elapsed <= 30) {
    const factor = totalGoals >= 2 || totalSot >= 3 ? 1.22 : 1.14;
    homeMultiplier *= factor;
    awayMultiplier *= factor;
    notes.push(`${elapsed}' 早球已出现，历史现场样本里后续大球路径明显上升，但仍需当前价不过热。`);
  }

  if (totalGoals >= 2 && elapsed >= 40 && elapsed <= 55) {
    homeMultiplier *= 1.12;
    awayMultiplier *= 1.12;
    notes.push(`${elapsed}' 已有 ${totalGoals} 球，半场附近多球样本更容易继续开放，大球/其他比分路径上修。`);
  }

  if (hasStats && elapsed >= 55 && goalDiff !== 0) {
    const trailingSide = goalDiff > 0 ? "away" : "home";
    const leaderSide = goalDiff > 0 ? "home" : "away";
    const trailingSot = Number(stats[trailingSide]?.shotsOnTarget);
    const trailingShots = Number(stats[trailingSide]?.shots);
    const leaderSot = Number(stats[leaderSide]?.shotsOnTarget);
    const leaderShots = Number(stats[leaderSide]?.shots);
    const trailingThreat = (Number.isFinite(trailingSot) && trailingSot >= 2)
      || (Number.isFinite(trailingShots) && trailingShots >= 8 && Number.isFinite(trailingSot) && trailingSot >= 1);
    if (trailingThreat) {
      if (trailingSide === "home") homeMultiplier *= 1.17;
      else awayMultiplier *= 1.17;
      notes.push(`${elapsed}' 落后一方已有射正/射门威胁，追平和 BTTS 路径按现场样本上修。`);
    }
    if (Number.isFinite(leaderSot) && Number.isFinite(trailingSot) && Number.isFinite(leaderShots) && Number.isFinite(trailingShots)) {
      if (leaderSot - trailingSot >= 3 && leaderShots - trailingShots >= 6) {
        if (leaderSide === "home") homeMultiplier *= 1.12;
        else awayMultiplier *= 1.12;
        notes.push(`${elapsed}' 领先方仍保持射正压制，不能简单按领先控节奏砍掉下一球路径。`);
      }
    }
  }

  homeMultiplier = clamp(homeMultiplier, 0.42, 1.55);
  awayMultiplier = clamp(awayMultiplier, 0.42, 1.55);
  return {
    ok: Math.abs(homeMultiplier - 1) >= 0.01 || Math.abs(awayMultiplier - 1) >= 0.01,
    homeMultiplier,
    awayMultiplier,
    notes: [...new Set(notes)].slice(0, 4)
  };
...
function liveRemainingGoalModel(match, live) {
  ...
  if (Number.isFinite(homeScore) && Number.isFinite(awayScore) && elapsed >= 55) {
    ...
    const sotDiff = statDiff(stats.home?.shotsOnTarget, stats.away?.shotsOnTarget);
    if (goalDiff >= 2) {
      const leadingPressure = shotDiff >= 8 || sotDiff >= 3;
      homeLambda *= leadingPressure ? 0.92 : 0.78;
      awayLambda *= 1.08;
      notes.push(`${elapsed}' ${match.homeName} 已领先 ${goalDiff} 球，领先方节奏管理；除非射门/射正继续压制，否则继续进球率降权。`);
    } else if (goalDiff <= -2) {
      const leadingPressure = shotDiff <= -8 || sotDiff <= -3;
      awayLambda *= leadingPressure ? 0.92 : 0.78;
      homeLambda *= 1.08;
      notes.push(`${elapsed}' ${match.awayName} 已领先 ${Math.abs(goalDiff)} 球，领先方节奏管理；除非射门/射正继续压制，否则继续进球率降权。`);
    } else if (goalDiff !== 0 && elapsed >= 75) {
      const leaderSide = goalDiff > 0 ? "home" : "away";
      const trailingSide = goalDiff > 0 ? "away" : "home";
      const trailingSot = Number(stats[trailingSide]?.shotsOnTarget);
      if (leaderSide === "home") homeLambda *= 0.86;
      else awayLambda *= 0.86;
      if (Number.isFinite(trailingSot) && trailingSot === 0) {
        if (trailingSide === "home") homeLambda *= 0.82;
        else awayLambda *= 0.82;
        notes.push(`${elapsed}' 落后一方仍 0 射正，追回路径降权。`);
      }
    }
  }
  ...
  const liveStageAdjustment = liveHistoricalStageAdjustment(match, live);
  if (liveStageAdjustment.ok) {
    homeLambda *= liveStageAdjustment.homeMultiplier;
    awayLambda *= liveStageAdjustment.awayMultiplier;
    notes.push(...liveStageAdjustment.notes);
  }
...
function remainingScoreProbabilityFromModel(remainingModel, homeAdditional, awayAdditional) {
  if (!Number.isInteger(homeAdditional) || !Number.isInteger(awayAdditional)) return 0;
  if (homeAdditional < 0 || awayAdditional < 0) return 0;
  return poisson(remainingModel.homeLambda, homeAdditional) * poisson(remainingModel.awayLambda, awayAdditional);
}

function liveCorrectScoreFairProbability(row, match, live, liveModel, remainingModel = null) {
  const homeScore = liveScoreNumber(live?.score?.home);
  const awayScore = liveScoreNumber(live?.score?.away);
  if (!Number.isFinite(homeScore) || !Number.isFinite(awayScore)) return row.liveProbability ?? row.modelProbability ?? 0;
  if (homeScore > row.homeGoals || awayScore > row.awayGoals) return 0;
  if (live?.completed) return homeScore === row.homeGoals && awayScore === row.awayGoals ? 1 : 0;
...
function liveScorePathContext(row, match, live, liveModel) {
  ...
  const homeShots = Number(stats.home?.shots);
  const awayShots = Number(stats.away?.shots);
  const homeCorners = Number(stats.home?.corners);
  const awayCorners = Number(stats.away?.corners);
  const homePossession = Number(stats.home?.possession);
  const awayPossession = Number(stats.away?.possession);
  ...
  const cornerDiff = Number.isFinite(homeCorners) && Number.isFinite(awayCorners) ? homeCorners - awayCorners : 0;
  ...
  return {
    ...
    sotDiff,
    cornerDiff,
    possessionDiff: roundTo(possessionDiff, 1),
    scoreKnown,
...
function pathTradingStance(row, context, liveEdge, dataQuality) {
  ...
  const price = Number(row.marketPrice);
  const fair = Number(row.liveProbability);
  const edgeValue = Number.isFinite(fair) && Number.isFinite(price) ? fair - price : liveEdge;
  const edgeKnown = Number.isFinite(edgeValue);
  if (context.targetReached) {
    if (edgeKnown && edgeValue >= 0.045 && context.remainingMinutes >= 18 && context.pressureSide !== (context.goalDiff >= 0 ? "home" : "away")) {
      return { action: "HOLD_TRIM", label: "已到目标，卖一半留尾仓", severity: "profit" };
    }
    if (edgeKnown && edgeValue <= -0.035) {
      return { action: "TAKE_PROFIT_FAST", label: "已到目标，价格偏高先锁利", severity: "profit" };
    }
    return { action: "TAKE_PROFIT", label: "已到目标，先止盈", severity: "profit" };
  }
  ...
  if (context.goalsNeeded === 1 && context.remainingMinutes >= 18 && edgeKnown && edgeValue >= 0.03 && context.pressureSide && (
    ...
    return { action: "PATH_ADD", label: "只差一球且压制，分批观察", severity: "good" };
  }
...
function scorePathPositionAdvice(row, context) {
  const price = Number(row.marketPrice);
  const fair = Number(row.liveProbability);
  const edgeValue = Number.isFinite(price) && Number.isFinite(fair) ? fair - price : null;
  const timePressure = context.remainingMinutes <= 15 ? "late" : context.remainingMinutes <= 35 ? "mid" : "early";
  let trimPct = 0;
  let keepPct = 0;
  let addPct = 0;
  let label = "不主动处理";
  ...
  if (!context.scoreKnown || !Number.isFinite(price) || !Number.isFinite(fair)) {
    return {
      label: "等待价格/比分",
      trimPct,
      keepPct,
      ...
      edge: edgeValue,
      timePressure,
      reasons: ["缺少当前价或比分，无法计算卖出/保留价值。"]
    };
  }

  if (context.targetReached) {
    const killRisk = clamp(1 - Math.exp(-((Number(row.nextGoalHazard) || 0))), 0, 0.92);
    if (edgeValue <= -0.04 || killRisk >= 0.55) {
      trimPct = 70;
      keepPct = 30;
      label = "价格/下一球风险偏高：先锁大部分";
    } else if (edgeValue >= 0.04 && timePressure !== "late") {
      trimPct = 35;
      keepPct = 65;
      label = "模型仍支持：卖一部分，保留时间衰减价值";
    } else if (timePressure === "late") {
      trimPct = 25;
      keepPct = 75;
      label = "临近终场：保留比分时间衰减";
    } else {
    ...
    reasons.push(`当前比分已是 ${context.targetText}，继续持有赚的是“无下一球”的时间衰减。`);
    reasons.push(`下一球杀死该比分的风险约 ${formatPercent(killRisk)}，edge ${formatPercent(edgeValue)}。`);
    return {
      label,
      trimPct,
      keepPct,
      addPct,
      edge: Number.isFinite(edgeValue) ? roundTo(edgeValue, 4) : null,
      timePressure,
      nextGoalKillRisk: roundTo(killRisk, 4),
      reasons
    };
  }

  if (context.goalsNeeded === 1 && edgeValue >= 0.04 && context.remainingMinutes >= 18) {
...
function correctScoreRowMarketUrl(row) {
  if (row?.marketSlug) return `https://polymarket.com/event/${row.marketSlug}`;
  if (row?.eventSlug) return `https://polymarket.com/event/${row.eventSlug}`;
  return "";
}

function buildLiveScorePathCandidates(baseRows, match, live, liveModel, currentRow, remainingModel) {
  const homeScore = liveScoreNumber(live?.score?.home);
  const awayScore = liveScoreNumber(live?.score?.away);
  if (!Number.isFinite(homeScore) || !Number.isFinite(awayScore)) return [];
  return (baseRows || [])
    .filter((row) => row && row.score && row !== currentRow)
    .map((row) => {
      const homeAdditional = Number(row.homeGoals) - homeScore;
      const awayAdditional = Number(row.awayGoals) - awayScore;
      if (homeAdditional < 0 || awayAdditional < 0) return null;
      const liveProbability = liveCorrectScoreFairProbability(row, match, live, liveModel, remainingModel);
      const marketPrice = Number(row.marketPrice);
      const edgeValue = Number.isFinite(marketPrice) ? liveProbability - marketPrice : null;
      const goalsNeeded = homeAdditional + awayAdditional;
      const pathType = homeAdditional === 0 && awayAdditional === 0 ? "current"
        : homeAdditional === 1 && awayAdditional === 0 ? "homeNext"
          : homeAdditional === 0 && awayAdditional === 1 ? "awayNext"
            : homeAdditional === 1 && awayAdditional === 1 ? "bothNext"
              : goalsNeeded === 1 ? "oneGoal"
                : "longer";
      const nextGoalImpact = homeAdditional === 0 && awayAdditional === 0
        ? "任意下一球会杀死该比分"
        : homeAdditional > 0 && awayAdditional === 0
          ? `${match.homeName} 下一球增强，${match.awayName} 下一球杀死`
          : awayAdditional > 0 && homeAdditional === 0
            ? `${match.awayName} 下一球增强，${match.homeName} 下一球杀死`
            : "双方都还要进球，路径更长";
      return {
        score: row.score,
        marketPrice: Number.isFinite(marketPrice) ? marketPrice : null,
        liveProbability: roundTo(liveProbability, 4),
        edge: Number.isFinite(edgeValue) ? roundTo(edgeValue, 4) : null,
        homeNeeded: homeAdditional,
        awayNeeded: awayAdditional,
        goalsNeeded,
        pathType,
        nextGoalImpact,
        action: Number.isFinite(edgeValue) && edgeValue >= 0.045 && goalsNeeded <= 1 ? "轮动候选"
          : Number.isFinite(edgeValue) && edgeValue >= 0.02 && goalsNeeded <= 2 ? "观察"
            : "不追",
        url: correctScoreRowMarketUrl(row)
      };
    })
    .filter(Boolean)
    .sort((a, b) => {
      const pathRank = { current: 0, homeNext: 1, awayNext: 1, oneGoal: 2, bothNext: 3, longer: 4 };
      const rankDiff = (pathRank[a.pathType] ?? 9) - (pathRank[b.pathType] ?? 9);
...
function livePathTimeDecay(row, context, remainingModel) {
  const noGoalProbability5 = Math.exp(-Math.max(0, (remainingModel.homeLambda + remainingModel.awayLambda) * Math.min(5, context.remainingMinutes) / Math.max(1, context.remainingMinutes)));
  const noGoalProbability10 = Math.exp(-Math.max(0, (remainingModel.homeLambda + remainingModel.awayLambda) * Math.min(10, context.remainingMinutes) / Math.max(1, context.remainingMinutes)));
  const totalHazard = Math.max(0, remainingModel.homeLambda + remainingModel.awayLambda);
  const direction = context.targetReached
    ? "no-goal-benefits"
    : context.goalsNeeded === 1
      ? "needs-next-goal"
      : "multi-event";
  const notes = [];
  if (context.targetReached) {
    notes.push(`5分钟无进球概率约 ${formatPercent(noGoalProbability5)}，10分钟无进球概率约 ${formatPercent(noGoalProbability10)}；无进球会抬高当前比分。`);
  } else if (context.goalsNeeded === 1) {
    notes.push(`路径还差 1 球，时间流逝会压低该比分；必须依赖目标方下一球。`);
  } else {
    notes.push(`路径还差 ${context.goalsNeeded} 球，时间每过 5-10 分钟都会明显侵蚀公允价。`);
  }
  notes.push(`剩余进球强度约 xG ${remainingModel.homeLambda.toFixed(2)}-${remainingModel.awayLambda.toFixed(2)}，总 hazard ${totalHazard.toFixed(2)}。`);
  return {
    direction,
    noGoal5: roundTo(noGoalProbability5, 4),
    noGoal10: roundTo(noGoalProbability10, 4),
    totalHazard: roundTo(totalHazard, 4),
    notes
  };
}

function buildScorePathTradeState(row, match, live, liveModel, dataQuality, baseRows = []) {
  const remainingModel = liveRemainingGoalModel(match, live);
  const context = liveScorePathContext(row, match, live, liveModel);
  const nextGoalHazard = remainingModel.homeLambda + remainingModel.awayLambda;
  row.nextGoalHazard = nextGoalHazard;
  const positionAdvice = scorePathPositionAdvice(row, context);
  const rotation = buildLiveScorePathCandidates(baseRows, match, live, liveModel, row, remainingModel);
  const timeDecay = livePathTimeDecay(row, context, remainingModel);
  const currentValue = {
    fair: roundTo(row.liveProbability || 0, 4),
    price: Number.isFinite(Number(row.marketPrice)) ? roundTo(Number(row.marketPrice), 4) : null,
    edge: Number.isFinite(Number(row.edge)) ? roundTo(Number(row.edge), 4) : null,
    maxBuyPrice: Number.isFinite(Number(row.maxBuyPrice)) ? roundTo(Number(row.maxBuyPrice), 4) : null,
    sellSignal: positionAdvice.trimPct >= 50 ? "trim-heavy"
      : positionAdvice.trimPct > 0 ? "trim-part"
        : positionAdvice.addPct > 0 ? "scale-small"
          : "hold-watch"
  };
  const nextGoal = [];
  if (context.scoreKnown) {
    if (context.targetReached) {
      nextGoal.push(`${match.homeName} 或 ${match.awayName} 下一球都会让 ${context.targetText} 失效；持有的是“无下一球”路径。`);
    } else {
      if (context.homeNeeded > 0) nextGoal.push(`${match.homeName} 下一球：${context.targetText} 路径增强。`);
      else nextGoal.push(`${match.homeName} 下一球：${context.targetText} 路径被杀死。`);
      if (context.awayNeeded > 0) nextGoal.push(`${match.awayName} 下一球：${context.targetText} 路径增强。`);
      else nextGoal.push(`${match.awayName} 下一球：${context.targetText} 路径被杀死。`);
    }
  }
  return {
    context,
    remainingModel: {
      homeLambda: roundTo(remainingModel.homeLambda, 3),
      awayLambda: roundTo(remainingModel.awayLambda, 3),
      notes: remainingModel.notes
    },
    currentValue,
    positionAdvice,
    rotation,
    ...
    nextGoal,
    dataLimits: [
      dataQuality.status !== "synced" ? "现场统计不足时，只能做比分时间模型，不能强补仓。" : "",
      "页面不知道你的真实持仓成本；止盈比例按通用仓位纪律给出，需要结合你的实际成本。"
    ].filter(Boolean)
  };
}
...
function buildCorrectScorePathPlan(row, match, live, liveModel, dataQuality, action) {
  const tradeState = row.tradeState || buildScorePathTradeState(row, match, live, liveModel, dataQuality, []);
  const context = tradeState.context || liveScorePathContext(row, match, live, liveModel);
  ...
  if (context.targetReached) {
    entry.push(`当前已经是 ${context.targetText}，这不是新买点；核心问题是卖多少、留多少。`);
    const trim = tradeState.positionAdvice?.trimPct || 0;
    const keep = tradeState.positionAdvice?.keepPct || 0;
    if (trim || keep) position.push(`持仓处理：先卖 ${trim}% 左右，保留 ${keep}% 左右吃时间衰减；不要一次全卖或全留。`);
    position.push(...(tradeState.positionAdvice?.reasons || []));
    takeProfit.push("价格跳升后先卖一部分回本金，剩余仓位只留作终场命中的尾仓。");
  ...
  rotation.push(...(tradeState.rotation || []).map((item) =>
    `${item.score}: ${item.action}，公允 ${formatPercent(item.liveProbability)}，当前 ${formatCents(item.marketPrice)}，edge ${formatPercent(item.edge)}；${item.nextGoalImpact}`
  ));
  ...
  return {
    ...
    context,
    tradeState,
    summary: `${stance.label}：${context.scoreText} -> ${context.targetText}，还差 ${context.goalsNeeded} 球，剩余约 ${context.remainingMinutes} 分钟；${tradeState.positionAdvice?.label || "按路径管理"}。`,
    entry: [...new Set(entry)].slice(0, 4),
    position: [...new Set(position.filter(Boolean))].slice(0, 5),
    ...
    nextGoal: [...new Set(nextGoal.filter(Boolean))].slice(0, 4),
    timeDecay: [...new Set(timeDecay.filter(Boolean))].slice(0, 4),
    rotation: [...new Set(rotation.filter(Boolean))].slice(0, 6),
...
function buildLiveCorrectScoreRecommendations(match, live, liveModel, dataQuality, correctScoreTradeGate = null) {
  ...
  return baseRows
    ...
    .sort((a, b) => {
      const aCtx = a.tradeState?.context || {};
      const bCtx = b.tradeState?.context || {};
      const currentDiff = Number(Boolean(bCtx.targetReached)) - Number(Boolean(aCtx.targetReached));
      if (currentDiff) return currentDiff;
      const neededDiff = (aCtx.goalsNeeded ?? 9) - (bCtx.goalsNeeded ?? 9);
...
function buildLiveRecommendationSummary(match, live, recommendations, dataQuality, liveModel, correctScoreRecommendations = []) {
  ...
  const correctScoreFocus = correctScoreRecommendations.find((rec) => rec.pathPlan?.context?.targetReached)
    || correctScoreRecommendations.find((rec) => rec.canConsider)
    || correctScoreRecommendations.find((rec) => rec.pathPlan?.tradeState?.positionAdvice?.addPct > 0)
    || null;
  if (correctScoreFocus) {
    const plan = correctScoreFocus.pathPlan || {};
    return {
      action: "watch",
      title: `实时球胆路径：${correctScoreFocus.score}`,
      summary: `${plan.summary || `${correctScoreFocus.score} 当前 ${formatCents(correctScoreFocus.marketPrice)}，实时模型 ${formatPercent(correctScoreFocus.liveProbability)}，edge ${formatPercent(correctScoreFocus.edge)}，建议上限 ${formatCents(correctScoreFocus.maxBuyPrice)}。`} 当前价 ${formatCents(correctScoreFocus.marketPrice)}，建议上限 ${formatCents(correctScoreFocus.maxBuyPrice)}。`,
      reasons: [
        ...(plan.entry || []).slice(0, 1),
        ...(plan.position || []).slice(0, 1),
        ...(plan.nextGoal || []).slice(0, 1),
        ...(plan.add || []).slice(0, 1),
        ...(plan.takeProfit || []).slice(0, 1),
        ...(liveModel.notes || []).slice(0, 2),
        ...(correctScoreFocus.risks || []).slice(0, 2)
...
function attachMarketCharts(matches, polymarket) {
  ...
  for (const match of matches) {
    ...
    sortRecommendationRows(match.recommendations, match);
...
function buildPhysicalMatchupAdjustment(match, options = {}) {
  ...
  const knockoutWeight = useRound16PlusLogic(match) ? 1.38 : isNationalTeamKnockoutMatch(match) ? 1.2 : 1;
...
async function fetchPolymarket(schedule = null, options = {}) {
  if (USE_DEMO_POLYMARKET) return demoPolymarket();

  const light = Boolean(options.light);
  const searches = light ? [] : buildPolymarketSearches(schedule);
  const eventSlugSearches = light ? [] : buildPolymarketEventSlugSearches(schedule);
  ...
  const [eventSlugResults, sportsPageResults, searchResults, soccerEvents] = await Promise.all([
    eventSlugSearches.length ? Promise.all(eventSlugSearches.map((search) => fetchPolymarketEventSlug(search))) : Promise.resolve([]),
    sportsPageSearches.length ? Promise.all(sportsPageSearches.map((search) => fetchPolymarketSportsPageMarkets(search))) : Promise.resolve([]),
    searches.length ? Promise.all(searches.map((search) => fetchPolymarketSearch(search))) : Promise.resolve([]),
    fetchPolymarketSoccerEvents({ timeoutMs: light ? LIGHT_FETCH_TIMEOUT_MS : FETCH_TIMEOUT_MS })
  ]);
  const soccerEventResult = polymarketSoccerEventsToMarketResult(soccerEvents);
  const derivativeResults = light ? [] : await mapLimit(
  ...
  const historyTokenLimit = light
    ? Math.min(POLYMARKET_HISTORY_TOKEN_LIMIT, POLYMARKET_LIGHT_HISTORY_TOKEN_LIMIT)
    : POLYMARKET_HISTORY_TOKEN_LIMIT;
  ...
  const tokenIds = [...new Set(normalizedMarkets
    ...
    .filter(Boolean))]
    .slice(0, historyTokenLimit);
  const history = tokenIds.length ? await fetchPriceHistory(tokenIds, {
    timeoutMs: light ? LIGHT_FETCH_TIMEOUT_MS : FETCH_TIMEOUT_MS
  }) : emptyHistory("没有可用 token_id");
...
async function fetchPolymarketSoccerEvents(options = {}) {
  const startedAt = Date.now();
  const timeoutMs = Number(options.timeoutMs || FETCH_TIMEOUT_MS);
  const urls = buildPolymarketSoccerEventUrls();
  const results = await Promise.all(urls.map((url) => timedFetchJson(url, { timeoutMs })));
...
function buildPolymarketSoccerEventUrls(nowMs = Date.now()) {
  const urls = [
  ...
  const endDateMin = new Date(nowMs - MATCH_SCHEDULE_LOOKBACK_DAYS * 86400000).toISOString();
  const endDateMax = new Date(shanghaiWindowEndMs(nowMs, MATCH_WINDOW_DAYS)).toISOString();
  for (const tagSlug of ["soccer", "games"]) {
    for (let page = 0; page < POLYMARKET_SOCCER_WINDOW_EVENT_PAGES; page += 1) {
      urls.push(`${POLYMARKET_GAMMA_API_BASE}/events?${new URLSearchParams({
        active: "true",
        closed: "false",
        limit: String(POLYMARKET_SOCCER_WINDOW_EVENT_LIMIT),
        offset: String(page * POLYMARKET_SOCCER_WINDOW_EVENT_LIMIT),
        tag_slug: tagSlug,
        order: "endDate",
        ascending: "true",
...
function polymarketSoccerEventsToMarketResult(soccerEvents) {
  ...
  const visibleBaseSlugs = new Set(selectPolymarketVisibleBaseEvents(soccerEvents.events || [])
    .map((event) => event.baseSlug || baseSoccerEventSlug(event.slug))
    .filter(Boolean));
  const markets = uniqueMarkets((soccerEvents.events || [])
    .filter((event) => visibleBaseSlugs.has(event.baseSlug || baseSoccerEventSlug(event.slug)))
    .flatMap((event) => {
...
async function fetchPriceHistory(tokenIds, options = {}) {
  ...
  const batchResults = await Promise.all(batches.map((batch) => fetchPriceHistoryBatch(batch, options)));
  const history = {};
...
async function fetchPriceHistoryBatch(tokenIds, options = {}) {
  ...
  const result = await timedFetchJson("https://clob.polymarket.com/batch-prices-history", {
    method: "POST",
    timeoutMs: Number(options.timeoutMs || FETCH_TIMEOUT_MS),
...
function compactRecommendation(rec = {}) {
  return {
    ...
    maxBuyPrice: rec.maxBuyPrice,
    disciplinedEdge: rec.disciplinedEdge,
    displaySortScore: rec.displaySortScore,
    odds: rec.odds,
  ...
  return {
    ...
    missingReason: payload.missingReason,
      comboStrategy: payload.comboStrategy ? {
        ...
        shape: payload.comboStrategy.shape,
...
function compactProbabilityBlock(probabilities = null) {
  if (!probabilities || typeof probabilities !== "object") return probabilities;
  return {
    ...probabilities,
    topScores: compactArray(probabilities.topScores, 10),
    topScoresFull: compactArray(probabilities.topScoresFull, 30)
  };
}

function compactModelV2(model = null) {
  if (!model || typeof model !== "object") return model;
  return {
    ...model,
    probabilities: compactProbabilityBlock(model.probabilities),
    drivers: compactArray(model.drivers, 8)
  };
}
...
function compactMatch(match = {}) {
  return {
    ...match,
    probabilities: compactProbabilityBlock(match.probabilities),
    modelV2: compactModelV2(match.modelV2),
