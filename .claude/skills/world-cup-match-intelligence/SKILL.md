...
- Match Polymarket derivative tokens strictly by market type: full-match totals such as O/U 2.5 must not match team totals, correct scores, player props, halves, corners, or spreads; BTTS must require `Both Teams To Score`/`BTTS` and must not match `first team to score`.
