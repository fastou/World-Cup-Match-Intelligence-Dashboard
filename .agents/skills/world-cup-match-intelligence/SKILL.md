...
- The page should show two separate scopes: a full next-three-day fixture schedule for visibility, and a primary tradable match tab set for the current Polymarket focus market date.
- The primary trading dashboard tabs should default to the current Polymarket focus market date, not the entire ESPN/FIFA schedule window. ESPN/FIFA schedule-only fixtures and other matched future markets can remain in backend context and the three-day schedule strip, but should not crowd the main tradable match tabs.
- Opportunity radar should use the same current Polymarket focus market date as the main trading tabs, rank by match lifecycle within that focused set, then diversify surfaced candidates across BTTS, totals, handicap, and match result. Do not let high-edge longshot moneyline rows crowd out structured market opportunities with live curves.
