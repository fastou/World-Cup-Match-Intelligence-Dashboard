...
- BTTS must have its own shape profile before recommendation: classify each match from the same score grid as open BTTS, BTTS watch, low-event balance, one-sided over, clean-sheet risk, unknown balance, or neutral. Show support/against reasons, 0-0 protection probability, BTTS+0-0 coverage, and the non-0-0 clean-sheet leak scores such as 1-0/2-0/0-1. League-level high BTTS rates are only low-weight priors; never recommend BTTS+0-0 unless the combined market cost is favorable and the leaked clean-sheet paths are acceptable.
- For open club leagues where rolling backtests show weak pre-match main-direction hit rate and elevated BTTS/Over 2.5, reduce favorite certainty through the same score grid, do not let Under 2.5 or BTTS No become main recommendations without strong lineup/live evidence, and allow low-priced underdog positive-edge rows to surface as small-watch candidates rather than hiding them as no-trade. Label this as review-driven observation, not a strong buy.
...
- For generalized soccer support, prioritize Polymarket `sports/soccer/games` plus Gamma date-window pagination, classify by tags/series/slug/title, reject esports before classification, keep derivative events under the base match, and never rely only on volume-sorted pages.
- Competition labels and models must distinguish national-team and club fixtures: UEFA club fixtures are club competitions, July/August UEFA club games are qualifying rounds when metadata is thin, and club fixtures need a club baseline from competition tier, `data/club-strength.json` league/club snapshots, and visible source confidence instead of reusing the national-team default rating. If a club is not covered by the snapshot, use a conservative competition plus league/country-strength fallback so unknown clubs do not all receive the same rating; label that fallback as low confidence.
- For club fixtures, do not display FIFA ranking or World Cup finals history as missing model inputs; mark those national-team layers as not applicable and show club strength, competition tier, recent form availability, and head-to-head status instead.
- Match Polymarket derivative tokens strictly by market type: full-match moneyline must not match halftime/first-half/second-half draw or winner markets; full-match totals such as O/U 2.5 must not match team totals, correct scores, player props, halves, corners, or spreads; BTTS must require `Both Teams To Score`/`BTTS` and must not match `first team to score`.
