...
- In-play elapsed time must trust explicit `minute`/`statusDetail` before status flags, and handicap picks must be recomputed from current score plus remaining-time goal distribution; late deep handicaps such as a favorite -1.5 while level near 80' must account for exact net-goal requirements and downgrade long tails.
