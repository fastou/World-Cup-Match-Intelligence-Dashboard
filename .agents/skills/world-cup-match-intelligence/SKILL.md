...
- Polymarket soccer schedule discovery must combine high-volume searches with date-window pagination. Do not rely on volume-sorted pages alone, because low-liquidity but real match events can be missing from the dashboard. Treat derivative events such as exact score, halftime result, first team to score, second half result, player props, and more-markets as market catalogs under the base match, not separate match cards.
